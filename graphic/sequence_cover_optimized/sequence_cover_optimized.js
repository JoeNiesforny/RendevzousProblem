...
function init(set = false) {
    ...
    layerLimit = layerLimit - Math.ceil(layerLimit / 8) + 1; // Optimized version
...
// According to pi* layers (for optimized version (O(d^3))):
// 1 - Q0, 2 - Q1, 3 - Q2, 4 - Q3, 5 - C4, 6 - Q4, 7 - C5
...
function computeLayer(layer) {
    return layer - (Math.floor((layer - (layer % 8)) / 4)) - (Math.floor(layer % 8) > 3 ? (Math.floor(layer % 8) > 5 ? 2 : 1) : 0);
}
...
function computeNewLayer(player) {
    if (player.layerIter % 8 > 3) { // Optimized for block sequence (z+4) = 8
        if (player.layerIter % 2 == 0 && player.layerIter != 0 && player.connectorD < 2) {
            connectorD(player);
            return;
        }
    }
