...
function init(set = false) {
    ...
    player = [{
        ...
        rotation : 0,
        d : 0,
        limit: Math.round(Math.random()*1000%roadLimit),
        diskX: 0, // position of new circle if run out of fuel
        diskY: 0,
    }, {
        x : Math.round(Math.random()*1000)%width,
        y : Math.round(Math.random()*1000)%height,
    ...
    if (set) {
        ...
        player[0].limit = document.getElementById('p1.limit').value - 0;
        player[1].limit = document.getElementById('p2.limit').value - 0;
    }

    for (i = 0; i < 2; i++) {
        player[i].diskX = player[i].x0 = player[i].x;
        player[i].diskY = player[i].y0 = player[i].y;
    }

    d_0 = d = distPlayer(player);
    ...
    document.getElementById('p1.limit').value = player[0].limit;
    document.getElementById('p2.limit').value = player[1].limit;
...
function draw() {
    ...
    for (i = 0; i < 2; i++) {
        if (fuel === 0) {
            console.log("player " + ((1-i)%2 + 1) + " run out of fuel at position ", player[(1-i)%2].x + ", " + player[(1-i)%2].y)
            SetNewCircle(player[i], player[(1-i)%2]);
            console.log("player " + i + " has new x00 " + player[i].x00 + ", y00 " + player[i].y00)
        }
        move(player[i]);
    }
...
function move(player) {
    if (player.d !== 0 && player.length < player.limit) {
...
function SetNewCircle(pWithFuel, pOutFuel) {
    fuel = 2;
    pWithFuel.diskX = pWithFuel.x0 + Math.round(pOutFuel.x) - pOutFuel.x0;
    pWithFuel.diskY = pWithFuel.y0 + Math.round(pOutFuel.y) - pOutFuel.y0;
    // New rotation speed
    pWithFuel.rotation = Math.abs(pWithFuel.speed) / d_0;
}
